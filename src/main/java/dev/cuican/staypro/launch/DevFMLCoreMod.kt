...
class DevFMLCoreMod : IFMLLoadingPlugin {
    ...
    init {
        val refMapFile = nextTempFile.apply {
            try {
                FileOutputStream(this).let {
                    it.write(DevFMLCoreMod::class.java.getResourceAsStream("/mixins.stay.refmap.json")!!.readBytes())
                    it.flush()
                    it.close()
                    this.deleteOnExit()
                }
        ...
        Gson().fromJson(
            String(
                DevFMLCoreMod::class.java.getResourceAsStream("/mixins.stay.json")!!.readBytes(),
