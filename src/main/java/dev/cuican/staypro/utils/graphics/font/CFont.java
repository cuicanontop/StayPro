package dev.cuican.staypro.utils.graphics.font;
...
public class CFont {
	...
	public CFont(CustomFont font, boolean antiAlias, boolean fractionalMetrics) {
		try {
			Font inputFont = Font.createFont(Font.TRUETYPE_FONT, CFont.class.getResourceAsStream(font.getFile())).deriveFont(font.getSize()).deriveFont(font.getType());// creat CFont with ttf
			this.font = inputFont;
			this.antiAlias = antiAlias;
			this.fractionalMetrics = fractionalMetrics;
			tex = setupTexture(inputFont, antiAlias, fractionalMetrics, this.charData);
