...
import dev.cuican.staypro.mixin.accessor.AccessorCPacketPlayer;
...
import static dev.cuican.staypro.utils.CrystalUtil.*;
import static org.lwjgl.opengl.GL11.GL_QUADS;

/**
 * Created by B_312 on 01/15/21
 * Updated by B_312 on 07/22/21
 * You are free to use it.But you should credit me.
 */
@Parallel(runnable = true)
@ModuleInfo(name = "StayAura", category = Category.COMBAT, description = "Place crystals to kill enemies")
public strictfp class StayAura extends Module {

    public static StayAura INSTANCE;

    Setting<String> page = setting("Page", "General", listOf("General", "Calculation", "Render"));

    //General
    Setting<Boolean> autoSwitch = setting("AutoSwitch", false).whenAtMode(page, "General").des("Auto switch crystals in MainHand");
    Setting<Integer> threadDelay = setting("ThreadDelay", 10, 1, 50).whenAtMode(page, "General").des("Calculation delay");
    Setting<Boolean> targetPlayer = setting("Players", true).whenAtMode(page, "General").des("Target players");
    Setting<Boolean> targetMob = setting("Mobs", false).whenAtMode(page, "General").des("Target mobs");
    Setting<Boolean> targetAnimal = setting("Animals", false).whenAtMode(page, "General").des("Target animals");
    Setting<Boolean> autoPlace = setting("Place", true).whenAtMode(page, "General").des("Place crystals");
    Setting<Boolean> autoExplode = setting("Explode", true).whenAtMode(page, "General").des("Explode crystals");
    Setting<Boolean> packetExplode = setting("PacketExplode", true).whenAtMode(page, "General").des("Inhibit crystal in crystal spawn packet");
    Setting<Boolean> predictExplode = setting("PredictExplode", false).whenAtMode(page, "General").des("Predict next crystal's entity id and explode it(Unsafe)");
    Setting<Integer> predictExplodeCount = setting("PredictHitCount", 2, 1, 20).whenAtMode(page, "General").whenTrue(predictExplode).des("Predict count");
    Setting<Boolean> clearClickDelay = setting("ClearClickDelay", true).whenAtMode(page, "General").des("Clear minecraft click cool down");
    Setting<Boolean> multiPlace = setting("MultiPlace", false).whenAtMode(page, "General").des("Multi place crystal");
    Setting<Double> attackSpeed = setting("AttackSpeed", 35d, 0, 50).whenAtMode(page, "General").des("Speed of breaking crystal");
    Setting<Double> placeSpeed = setting("PlaceSpeed", 35d, 0d, 50).whenAtMode(page, "General").des("Speed of placing crystal");
    Setting<Double> distance = setting("Distance", 7.0D, 0D, 8D).whenAtMode(page, "General").des("Calculation working range");
    Setting<Double> placeRange = setting("PlaceRange", 4.5D, 0D, 8D).whenAtMode(page, "General").des("Max place range");
    Setting<Double> hitRange = setting("HitRange", 5.5D, 0D, 8D).whenAtMode(page, "General").des("Max break range");
    Setting<Boolean> rotate = setting("Rotate", true).whenAtMode(page, "General").des("Rotation packets and animation");
    Setting<Boolean> rayTrace = setting("RayTrace", false).whenAtMode(page, "General").des("Ray trace block side");
    //Calculation
    Setting<Boolean> oneThirtyPlace = setting("1.13Place", false).whenAtMode(page, "Calculation").des("Place mode in 1.13 server");
    Setting<Boolean> wall = setting("Wall", true).whenAtMode(page, "Calculation").des("Wall calculation");
    Setting<Double> wallRange = setting("WallRange", 3.0, 0d, 5d).whenAtMode(page, "Calculation").des("Wall calculation range");
    Setting<Boolean> noSuicide = setting("NoSuicide", true).whenAtMode(page, "Calculation").des("No suicide");
    Setting<Boolean> facePlace = setting("FacePlace", true).whenAtMode(page, "Calculation").des("Force place while enemies are nearly dying");
    Setting<Double> blastHealth = setting("MinHealthFace", 10d, 0d, 20d).whenAtMode(page, "Calculation").whenTrue(facePlace).des("When enemy's health is lower than this then we starting face place");
    Setting<Double> placeMinDmg = setting("PlaceMinDamage", 4.5d, 0d, 20d).whenAtMode(page, "Calculation").des("Each placement must make damage on enemy higher than this");
    Setting<Double> placeMaxSelf = setting("PlaceMaxSelf", 10d, 0d, 36d).whenAtMode(page, "Calculation").des("Each placement must make damage on self lower than this");
    Setting<String> attackMode = setting("HitMode", "Smart", listOf("Smart", "Always")).whenAtMode(page, "Calculation").des("Always or Smart(With break calculation)");
    Setting<Double> breakMinDmg = setting("BreakMinDmg", 4.5, 0.0, 36.0).whenAtMode(page, "Calculation").whenAtMode(attackMode, "Smart").des("Each break must make damage on enemy higher than this");
    Setting<Double> breakMaxSelf = setting("BreakMaxSelf", 12.0, 0.0, 36.0).whenAtMode(page, "Calculation").whenAtMode(attackMode, "Smart").des("Each break must make damage on self lower than this");
    Setting<Boolean> lethalTryIt = setting("LethalTryIt", true).whenAtMode(page, "Calculation").whenAtMode(attackMode, "Smart").des("If we can pop enemy's totem,we take a risk");
    Setting<Boolean> ghostHand = setting("GhostHand", false).whenAtMode(page, "Calculation").des("Ghost hand means that you don't need to switch to crystal to place");
    Setting<Boolean> pauseWhileEating = setting("PauseWhileEating", false).whenAtMode(page, "Calculation").des("Pause StayAura while you are eating");
    Setting<Boolean> clientSide = setting("ClientSideConfirm", false).whenAtMode(page, "Calculation").des("Client side remove crystal that you've attacked");
    //Render
    Setting<Boolean> renderDmg = setting("RenderDamage", false).whenAtMode(page, "Render").des("Render damage on targeted block");
    Setting<String> renderMode = setting("RenderBlock", "Full", listOf("Solid", "Up", "UpLine", "Full", "Outline", "OFF")).whenAtMode(page, "Render").des("The render type");
    Setting<Boolean> syncGUI = setting("SyncGui", false).whenAtMode(page, "Render").des("Synchronize color with GUI");
    Setting<Integer> red = setting("Red", 255, 0, 255).whenAtMode(page, "Render").whenFalse(syncGUI).des("Custom color Red");
    Setting<Integer> green = setting("Green", 0, 0, 255).whenAtMode(page, "Render").whenFalse(syncGUI).des("Custom color Green");
    Setting<Integer> blue = setting("Blue", 0, 0, 255).whenAtMode(page, "Render").whenFalse(syncGUI).des("Custom color Blue");
    Setting<Integer> transparency = setting("Alpha", 26, 0, 255).whenAtMode(page, "Render").des("Custom transparency");
    Setting<Boolean> rainbow = setting("Rainbow", false).whenAtMode(page, "Render").whenFalse(syncGUI).des("Rainbow dynamic color");
    Setting<Float> rainbowSpeed = setting("RGB Speed", 1.0f, 0.0f, 10.0f).whenAtMode(page, "Render").whenFalse(syncGUI).des("Rainbow color change speed");
    Setting<Float> saturation = setting("Saturation", 0.65f, 0.0f, 1.0f).whenAtMode(page, "Render").whenFalse(syncGUI).des("Rainbow color saturation");
    Setting<Float> brightness = setting("Brightness", 1.0f, 0.0f, 1.0f).whenAtMode(page, "Render").whenFalse(syncGUI).des("Rainbow color brightness");
    ...
    private void placeCrystal(BlockPos targetBlock, EnumHand enumHand) {
        ...
        if (placeDelayRun(placeSpeed.getValue())) {
            ...
            if (predictExplode.getValue()) {
                ...
                TaskManager.repeat(predictExplodeCount.getValue(), () -> {
                    if (syncedId != -1) {
                        CPacketUseEntity attackPacket = new CPacketUseEntity();
    ...
    public static class LocalTarget {
        ...
        public synchronized CrystalTarget getPlaceTarget(boolean shouldIgnoreEntity) {
            synchronized (placeTargets) {
                ...
                return StayAura.INSTANCE.calculator(StayAura.INSTANCE.shouldIgnoreEntity.get());
            }
        }
