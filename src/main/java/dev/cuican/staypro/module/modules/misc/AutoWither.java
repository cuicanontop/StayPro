...
@ModuleInfo(name = "AutoWither", category = Category.MISC)

public class AutoWither extends Module {

    private static boolean isSneaking;
    private final Setting<String> useMode = setting("Mode", "SINGLE",listOf( "SINGLE", "SPAM"));
    private final Setting<Float> placeRange = setting("Range", 3f, 1, 6);
    private final Setting<Integer> delay = setting("Delay", 20, 12, 100).whenAtMode(useMode, "SPAM");
    private final Setting<Boolean> rotate = setting("Rotate", false);
    private final Setting<Boolean> debug = setting("Debug", true);
    ...
    public void onTick() {
        ...
        if (this.buildStage == 1) {
            ...
            if (!this.checkBlocksInHotbar()) {
                ChatUtil.sendNoSpamMessage("no blocks in hotbar");
                return;
            }
            ...
            if (noPositionInArea) {
                if (this.useMode.getValue().equals(UseMode.SINGLE)) {
                    if (this.debug.getValue().booleanValue()) {
                        ChatUtil.sendNoSpamMessage("[AutoSpawner] " + ChatFormatting.RED.toString() + "Position not valid, disabling.");
                    }
                    this.disable();
                }
                return;
